...
const setup = async () => {
  describe("Vault with Yearn Token", () => {
    ...
    before(async () => {
      ...
      console.log(
        `await token:${ethers.utils.formatEther(
          await yDAItoken.balanceOf(YDAIWhale)
        )}`
      );
      await yDAItoken
        .connect(tokenHolder)
    ...
    describe("handleDeposit()", () => {
      ...
      it("Transfer asset from recipient to vault", async () => {
        ...
        const whaleBal = await res.token.balanceOf(DAIWhale);
        await res.token
          .connect(res.whale)
          .transfer(yDaiVault.address, whaleBal);
        ...
        console.log(
          ` 
          initial dai deposited:${ethers.utils.formatEther(
            initialPricePerShare.mul(tokenDeposited)
          )}
          initialPricePerShare:${ethers.utils.formatEther(initialPricePerShare)}
          yVaultDaiBalBefore bal:${ethers.utils.formatEther(
            yVaultDaiBalBefore
          )} 
          vaultDaiBalAfter bal:${ethers.utils.formatEther(yVaultDaiBalAfter)} 
          
          whaleBal bal:${ethers.utils.formatEther(whaleBal)} 
          `
        );
        ...
        console.log(
          ...
          current dai deposited value:${pricePerShare
    ...
    describe("handleWithdrawal()", () => {
      ...
      it("Transfer asset from vault to recipient", async () => {
        ...
        console.log(
          ` accDaiBalBefore:${ethers.utils.formatEther(accDaiBalBefore)}
          `
        );
        await yDaiVault.connect(account0)["withdraw(uint256)"](tokenDeposited);
        const accDaiBalAfter = await daiToken.balanceOf(account0.address);
        console.log(
          `  
          accDaiBalAfter:${ethers.utils.formatEther(accDaiBalAfter)}
          toETHNumber(precision):${toETHNumber(precision)}
          `
        );
        ...
        expect(daiWithdraw).to.be.approximately(
          toETHNumber(tokenDeposited) * percentageIncrease,
